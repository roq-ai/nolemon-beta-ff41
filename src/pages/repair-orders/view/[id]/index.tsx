...
import { getRepairOrderById } from 'apiSdk/repair-orders';
      ...
      <Box bg="white" p={4} rounded="md" shadow="md">
        <Flex justifyContent="space-between" mb={4}>
          <Text as="h1" fontSize="2xl" fontWeight="bold">
            Repair Order Detail View
          </Text>
          {hasAccess('repair_order', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
            <NextLink href={`/repair-orders/edit/${data?.id}`} passHref legacyBehavior>
              <Button
                ...
                <Flex alignItems="center" mb={4}>
                  ...
                  <Text fontSize="md" as="span" ml={3}>
                    <Link as={NextLink} href={`/technicians/view/${data?.technician?.id}`}>
                      {data?.technician?.efficiency_rating}
                    </Link>
                  </Text>
                </Flex>
              )}
